...
#include <queue>
using namespace std;
int minutes_to_print(priority_queue<int>printer_queue_s, queue<pair<int, int>> printer_queue, int m){
    int count = 0;
    while(!printer_queue.empty()){
        pair<int, int> current = printer_queue.front();
        int current_max = printer_queue_s.top();
        if(current.first != current_max){
            printer_queue.pop();
